...
2. In Render: **New → Blueprint**, point it at your repo. Render will read
   `render.yaml` and provision the web service + persistent disk automatically.
...
4. Add a **Disk** (e.g. 1GB, mount path `/opt/render/project/data`) so the
   SQLite database persists across deploys — Render's filesystem is
   otherwise ephemeral.
5. Set env var `DATABASE_URL=sqlite:////opt/render/project/data/cfp_agent.db`
   (matching your mount path), and `REFRESH_INTERVAL_HOURS` as desired.

> **Note on free-tier Render services:** free web services spin down after
> inactivity, which also pauses the background scheduler. If you need
> guaranteed periodic refreshes even with no traffic, either upgrade to a
> paid instance type, or instead call `POST /api/refresh` from an external
> cron service (e.g. Render Cron Jobs, GitHub Actions on a schedule, or
> cron-job.org) hitting your deployed URL.
